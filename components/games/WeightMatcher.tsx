  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 border border-purple-100 shadow-sm space-y-6">
      <div className="flex justify-between items-center">
        <span className="text-sm font-bold text-purple-600 bg-purple-50 px-3 py-1 rounded-full">Score: {score}/{total}</span>
        <span className="text-2xl">⚖️</span>
      </div>

      <p className="text-center font-semibold text-slate-600">Make the scale balance! Find <span className="text-purple-700 font-extrabold text-lg">{round.target}</span></p>
      ...
      <div className="flex justify-center py-4">
        <div className="relative w-72 h-48">
          ...
          {/* Beam — tilts based on weight difference */}
          ...
          {/* Labels */}
          <div className="absolute left-2 top-[105px] text-xs text-slate-400 font-semibold w-20 text-center">Target</div>
          <div className="absolute right-2 top-[105px] text-xs text-purple-400 font-semibold w-20 text-center">Your pick</div>
        </div>
      </div>
      ...
      {feedback && (
        <div className={`text-center text-2xl font-extrabold animate-bounce ${feedback === 'correct' ? 'text-green-500' : 'text-red-400'}`}>
          {feedback === 'correct' ? '⚖️ Balanced!' : `❌ It was ${round.target}`}
        </div>
      )}
    </div>
  );
