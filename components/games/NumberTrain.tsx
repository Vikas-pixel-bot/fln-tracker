...
function makeRound() {
  ...
  const gapPos = Math.floor(Math.random() * 3) + 1;
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 border border-red-100 shadow-sm space-y-6">
      <div className="flex justify-between items-center">
        <span className="text-sm font-bold text-red-600 bg-red-50 px-3 py-1 rounded-full">गुण: {score}/{total}</span>
        <span className="text-2xl">🚂</span>
      </div>

      <p className="text-center font-bold text-slate-700">गाडीतील गायब संख्या कोणती?</p>
      ...
      {feedback && (
        <div className={`text-center text-xl font-extrabold animate-bounce ${feedback === 'correct' ? 'text-green-500' : 'text-red-400'}`}>
          {feedback === 'correct' ? '🚂 शाब्बास! बरोबर!' : `❌ उत्तर: ${round.answer}`}
        </div>
      )}
    </div>
  );
